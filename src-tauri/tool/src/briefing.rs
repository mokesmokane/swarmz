//! What an agent session (Claude or Codex) in a tile is told at start (conversation cards spec §4.1, conductor
...
- Each row in `fleet` carries the tile's board in brief (`board`: goal, now, next, plan progress, branch and base, flags); `~/.swarmz/bin/swarmz board --tile <id> --get` reads a whole board. When the user asks how the work streams stand against each other (which are behind, which overlap, which one another has superseded), compare the boards, then check the git state in the tiles' folders (branch, what it is based on, ahead/behind the shared integration branch, uncommitted files) before you answer.
...
- `~/.swarmz/bin/swarmz send <tile> -- "instruction"` tells a tile to do something (the line is marked as coming from you). `pending <tile>` and `answer <tile> …` handle a tile's permission or question, but never answer a permission the user did not tell you to. `new` and `close` start and stop tiles; `restart <tile>` starts whatever is not running: a stopped tile afresh, or its agent (Claude or Codex) again, resuming its conversation, in a tile whose agent has exited (never type a `claude` or `codex` command with `send`: the line is marked as yours and the shell would not run it). `new --folder <dir> --agent codex` starts a Codex tile instead of a Claude one.
...
- `~/.swarmz/bin/swarmz ask <tile> -- "question"` and `send <tile> -- "instruction"` reach the tiles directly under you; answers arrive later as prompts starting `[<its title>]`. `pending`, `answer` and `close` work on them too, `restart <tile>` brings back a tile or an agent that has exited (resuming its conversation; never type a `claude` or `codex` command with `send`), and a tile you start with `new --folder <dir>` is yours. Never answer a permission nobody told you to.

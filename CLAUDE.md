...
The tool is also the phone's whole interface (phone spec §4): `src-tauri/tool/src/commands.rs` has one function per subcommand over small pure modules (`agent` folds `events.log` the same way as `agentState.ts`, checked by the shared `tests/fixtures/agent-status.json`; `transcript` normalises Claude transcripts; `screen` is the holder's `vt100` model behind `Screen` frames; `dialog` reads the permission dialog; `tiles` lists the tiles homed on this Mac; `newtile`, `phone`, `gate`). The holder's screen decides permission state (spec §4.2): a permission dialog on the visible screen means blocked whatever the (asynchronous) hook log says, and no dialog ends a logged permission block; `pending`/`answer` use the screen's summary. Holders whose metadata has no `build` predate `Screen`: `output`/`pending`/`answer` refuse them with `old_session`, and `ls`/`watch` keep the fold. Tool readers use `workspace::read_from`, which never moves a broken file aside; only writers (`new`, the `__keep-def` helper `new` starts to re-add its def for 30 s) use `load_from`. CLI tests in `src-tauri/tool/tests/cli.rs` run the real binary with `HOME` under `/tmp/szc-*` and `SWARMZ_MACHINE` set (which also skips Tailscale). Phone keys are forced through `swarmz ssh-gate` and limited to tailnet addresses (`from=`).

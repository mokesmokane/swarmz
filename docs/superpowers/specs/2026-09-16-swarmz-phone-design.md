...
| `send <tile> [--] <text>` | `{sent:true}`; asks the holder's `Info` first and types the text as a bracketed paste when the program on screen has turned bracketed paste on (or the holder cannot say), else as plain text, then Enter as a separate write 50 ms later; when the holder's screen then still shows the text in Claude's input box (Claude took the Enter as part of the paste), Enter is pressed again, up to three times at growing pauses, and the reply's `resent` counts those, with `submitted: false` when the text is still there after the last one (absent for a holder without a screen) (amended 2026-09-23); everything after a lone `--` is text, never an option, so a command that itself starts with `--` can be sent; the pasted text keeps only newline, tab and printable characters, so it can never end the paste early or smuggle escape sequences of its own |
